...
namespace GaussianSplatting.Runtime
{
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        void SetAssetDataOnCS(CommandBuffer cmb, KernelIndices kernel)
        {
            ...
            // ── 绑定语义缓冲 ─────────────────────────────────────
            if (m_SemanticController != null && m_SemanticController.gpuVisibility != null)
            {
                cmb.SetComputeBufferParam(cs, kernelIndex, Props.SemanticVisibility, m_SemanticController.gpuVisibility);
                cmb.SetComputeBufferParam(cs, kernelIndex, Props.SemanticTints, m_SemanticController.gpuTints);
                cmb.SetComputeIntParam(cs, Props.SemanticLayerCount, m_SemanticController.semanticCount);
            }
            else
            {
                // 无语义控制器时，语义层数设为 0，Shader 侧跳过语义逻辑
                cmb.SetComputeIntParam(cs, Props.SemanticLayerCount, 0);
            }
        }
        ...
        internal void SortPoints(CommandBuffer cmd, Camera cam, Matrix4x4 matrix)
        {
            ...
            if (m_SemanticController != null && m_SemanticController.gpuVisibility != null)
            {
                cmd.SetComputeBufferParam(m_CSSplatUtilities, (int)KernelIndices.CalcDistances, Props.SemanticVisibility, m_SemanticController.gpuVisibility);
                cmd.SetComputeIntParam(m_CSSplatUtilities, Props.SemanticLayerCount, m_SemanticController.semanticCount);
            }
            else
            {
                cmd.SetComputeIntParam(m_CSSplatUtilities, Props.SemanticLayerCount, 0);
            }
