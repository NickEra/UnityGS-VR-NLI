...
namespace GaussianSplatting.Runtime
{
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        // Dummy buffers：当没有 SemanticController 时，仍需绑定到 Compute Shader
        // 避免 Unity 报 "Property (_SemanticVisibility) is not set"
        GraphicsBuffer m_DummySemanticVisibility;
        GraphicsBuffer m_DummySemanticTints;
        ...
        internal static class Props
        {
            ...
            // Semantic control buffers
            public static readonly int SemanticVisibility = Shader.PropertyToID("_SemanticVisibility");
            public static readonly int SemanticTints = Shader.PropertyToID("_SemanticTints");
            public static readonly int SemanticLayerCount = Shader.PropertyToID("_SemanticLayerCount");
        }
        ...
        /// <summary>获取关联的语义控制器（可能为 null）</summary>
        public SemanticController semanticController => m_SemanticController;
        SemanticController m_SemanticController;
        ...
        public void UpdateRessources()
        {
            ...
            // ── 语义改造核心改动 ──────────────────────────────────
            // 始终加载所有层的数据到 GPU，可见性控制由 SemanticController 在 GPU 侧完成。
            // 原始代码只加载 activeLayers，现在加载全部。
            m_SplatCount = asset.splatCount; // 所有层的 splat 之和
            ...
            // 遍历所有层（不再过滤 activeLayers）
            foreach (var layerAssets in asset.LayerData)
            {
            ...
            // ── 构建 per-splat layer ID 数组 ─────────────────────
            var layerIdPerSplat = new uint[m_SplatCount];
            int globalSplatIdx = 0;
            var cachedLayerInfo = asset.layerInfo; // 缓存避免循环内重复 ToDictionary()

            // ── 一致性断言：拦截训练端导出与 Asset 元数据不匹配的情况 ──
            {
                int layerInfoSum = 0;
                foreach (var kv in cachedLayerInfo)
                    layerInfoSum += kv.Value;
                if (layerInfoSum != m_SplatCount)
                    Debug.LogError($"[GaussianSplat] Semantic data inconsistency: " +
                        $"sum(layerInfo)={layerInfoSum} != splatCount={m_SplatCount}. " +
                        $"Layer ID assignment will be wrong. Check PLY export / merge_3dgs.py output.");

                // 逐层校验：posData 字节数 / posStride 应等于 layerInfo 声明的 splatCount
                int posStride = GaussianSplatAsset.GetVectorSize(asset.posFormat);
                if (posStride > 0)
                {
                    foreach (var layerAssets in asset.LayerData)
                    {
                        int layerId = layerAssets.layer;
                        int posSplats = (int)layerAssets.m_PosData.dataSize / posStride;
                        int declaredCount = cachedLayerInfo.TryGetValue(layerId, out int cnt) ? cnt : -1;
                        if (posSplats != declaredCount)
                            Debug.LogWarning($"[GaussianSplat] Layer {layerId}: posData implies {posSplats} splats " +
                                $"but layerInfo declares {declaredCount}. Semantic IDs may be misaligned.");
                    }
                }
            }
            ...
            foreach (var layerAssets in asset.LayerData)
            {
                ...
                // 填充该层的 splat → layer ID 映射
                int layerId = layerAssets.layer;
                int layerSplatCount = cachedLayerInfo.TryGetValue(layerId, out int cnt) ? cnt : 0;
                for (int i = 0; i < layerSplatCount && globalSplatIdx < m_SplatCount; i++)
                {
                    // 钳位到 MAX_SEMANTIC_LAYERS - 1 防止 GPU 越界
                    layerIdPerSplat[globalSplatIdx] = (uint)Mathf.Min(layerId, SemanticController.kMaxSemanticLayers - 1);
                    globalSplatIdx++;
                }
            }
            ...
            if (asset.ClusteredSHData != null)
            {
                shDataArr.Dispose();
                shDataArr = asset.ClusteredSHData.GetData<byte>();
            }
            ...
            // ── 创建并填充 m_GpuLayerData ────────────────────────
            m_GpuLayerData = new GraphicsBuffer(GraphicsBuffer.Target.Structured, m_SplatCount, sizeof(uint))
            { name = "GaussianLayerData" };
            m_GpuLayerData.SetData(layerIdPerSplat);
            ...
            // ── 初始化 SemanticController ─────────────────────────
            if (m_SemanticController == null)
                m_SemanticController = GetComponent<SemanticController>();
            if (m_SemanticController != null)
                m_SemanticController.InitializeFromAsset(m_Asset);

            // ── 绑定语义缓冲到所有引用它的 kernel ──────────────────
            // ComputeShader 全局声明了 _SemanticVisibility/_SemanticTints，
            // Unity 要求所有声明了这些属性的 kernel 都必须有绑定，
            // 即使 _SemanticLayerCount==0 跳过语义逻辑也不行。
            // 所以无论是否有 SemanticController，都必须绑定（没有就绑 dummy）。
            EnsureDummySemanticBuffers();

            var vis = ActiveSemanticVisibility;
            var tints = ActiveSemanticTints;
            int layerCount = ActiveSemanticLayerCount;

            m_CSSplatUtilities.SetBuffer((int)KernelIndices.CalcDistances, Props.SemanticVisibility, vis);
            m_CSSplatUtilities.SetBuffer((int)KernelIndices.CalcDistances, Props.SemanticTints, tints);
            m_CSSplatUtilities.SetBuffer((int)KernelIndices.CalcViewData, Props.SemanticVisibility, vis);
            m_CSSplatUtilities.SetBuffer((int)KernelIndices.CalcViewData, Props.SemanticTints, tints);
            m_CSSplatUtilities.SetInt(Props.SemanticLayerCount, layerCount);

            if (m_SemanticController != null)
                m_SemanticController.UploadIfDirty();
        ...
        public void UpdateSortingType(GpuSorting.SortType sortType)
        {
            ...
            // Guard: GpuSorting constructor (DeviceRadixSort) calls new LocalKeyword(cs, ...)
            // which crashes with NullReferenceException if cs is null.
        ...
        void SetAssetDataOnCS(CommandBuffer cmb, KernelIndices kernel)
        {
            ...
            // ── 绑定语义缓冲（始终绑定，无 SemanticController 时用 dummy）────
            cmb.SetComputeBufferParam(cs, kernelIndex, Props.SemanticVisibility, ActiveSemanticVisibility);
            cmb.SetComputeBufferParam(cs, kernelIndex, Props.SemanticTints, ActiveSemanticTints);
            cmb.SetComputeIntParam(cs, Props.SemanticLayerCount, ActiveSemanticLayerCount);
        }
        ...
        /// <summary>
        /// 确保 dummy 语义缓冲存在。
        /// ComputeShader 中全局声明了 _SemanticVisibility / _SemanticTints，
        /// 即使 _SemanticLayerCount==0 跳过语义逻辑，Unity 仍要求所有声明的 buffer 已绑定。
        /// </summary>
        void EnsureDummySemanticBuffers()
        {
            if (m_DummySemanticVisibility == null)
            {
                // 1 个元素，值为 1（可见），任何意外读取不会导致隐藏
                m_DummySemanticVisibility = new GraphicsBuffer(GraphicsBuffer.Target.Structured, 1, sizeof(uint))
                { name = "DummySemanticVisibility" };
                m_DummySemanticVisibility.SetData(new uint[] { 1u });
            }
            if (m_DummySemanticTints == null)
            {
                // 1 个 float4 元素，值为 (1,1,1,1)（白色 = 不改变颜色）
                m_DummySemanticTints = new GraphicsBuffer(GraphicsBuffer.Target.Structured, 1, 4 * sizeof(float))
                { name = "DummySemanticTints" };
                m_DummySemanticTints.SetData(new Vector4[] { new Vector4(1, 1, 1, 1) });
            }
        }

        void DisposeDummySemanticBuffers()
        {
            DisposeBuffer(ref m_DummySemanticVisibility);
            DisposeBuffer(ref m_DummySemanticTints);
        }

        /// <summary>
        /// 获取当前应绑定的语义可见性缓冲（有 SemanticController 则用真的，否则用 dummy）
        /// </summary>
        GraphicsBuffer ActiveSemanticVisibility =>
            (m_SemanticController != null && m_SemanticController.gpuVisibility != null)
                ? m_SemanticController.gpuVisibility
                : m_DummySemanticVisibility;

        /// <summary>
        /// 获取当前应绑定的语义着色缓冲
        /// </summary>
        GraphicsBuffer ActiveSemanticTints =>
            (m_SemanticController != null && m_SemanticController.gpuTints != null)
                ? m_SemanticController.gpuTints
                : m_DummySemanticTints;

        /// <summary>
        /// 获取当前语义层数（无控制器时返回 0，GPU 侧跳过语义逻辑）
        /// </summary>
        int ActiveSemanticLayerCount =>
            (m_SemanticController != null) ? m_SemanticController.semanticCount : 0;
        ...
        internal void SortPoints(CommandBuffer cmd, Camera cam, Matrix4x4 matrix)
        {
            ...
            // ── 绑定语义缓冲（始终绑定，无 SemanticController 时用 dummy）────
            cmd.SetComputeBufferParam(m_CSSplatUtilities, (int)KernelIndices.CalcDistances, Props.SemanticVisibility, ActiveSemanticVisibility);
            cmd.SetComputeBufferParam(m_CSSplatUtilities, (int)KernelIndices.CalcDistances, Props.SemanticTints, ActiveSemanticTints);
            cmd.SetComputeIntParam(m_CSSplatUtilities, Props.SemanticLayerCount, ActiveSemanticLayerCount);
