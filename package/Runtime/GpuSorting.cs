...
namespace GaussianSplatting.Runtime
{
    ...
    public class GpuSorting
    {
        ...
        {
            // Guard: 如果 cs 为 null 或已销毁（Unity fake-null），退化为不排序
            if (!cs)
            {
                activeType = SortType.None;
                return;
            }
    ...
    public class DeviceRadixSort
    {
        ...
        {
            m_CS = cs;
            // 关键：cs 为 null 或 Unity fake-null 时，不能调用 new LocalKeyword(cs, ...)
            // Unity Object 的隐式 bool 转换能检测"已销毁"情况
            if (!cs)
            {
                m_Valid = false;
                return;
