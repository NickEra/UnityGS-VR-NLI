    ...
    public class GaussianSplatRendererEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (gs.HasValidAsset)
            {
                ...
                // ── 语义层控制面板 ────────────────────────────────
                var semantic = gs.semanticController;
                if (semantic != null && semantic.activeLayerCount > 0)
                {
                    GUILayout.Label("Semantic Layer Control", EditorStyles.boldLabel);
                    ...
                    {
                        semantic.SetAllVisibility(false);
                        EditorUtility.SetDirty(gs);
                    }
                    if (GUILayout.Button("Reset Tints", EditorStyles.miniButtonRight))
                    {
                    ...
                    for (int idx = 0; idx < semantic.activeLayerCount; idx++)
                    {
                        int i = semantic.activeLayerIds[idx];
                        EditorGUILayout.BeginHorizontal();
                        
                        bool visible = semantic.GetVisibility(i);
                        string label = semantic.GetSemanticName(i);
                        int splatCount = semantic.GetLayerSplatCount(i);
                        var check = EditorGUILayout.Toggle(visible, GUILayout.Width(16));
                        if (check != visible)
                        {
                            semantic.SetVisibility(i, check);
                            EditorUtility.SetDirty(gs);
                        }
                        
                        EditorGUILayout.LabelField(label, GUILayout.MinWidth(80));
                        
                        Color tint = semantic.GetTint(i);
                        var newTint = EditorGUILayout.ColorField(GUIContent.none, tint, 
                            false, true, false, GUILayout.Width(50));
                        if (newTint != tint)
                        {
                            semantic.SetTint(i, newTint);
                            EditorUtility.SetDirty(gs);
                        }
                        
                        if (GUILayout.Button("Solo", EditorStyles.miniButton, GUILayout.Width(36)))
                        {
                            semantic.IsolateLayer(i);
                            EditorUtility.SetDirty(gs);
                        }
                ...
                {
                    // SemanticController exists but not yet initialized
                    GUILayout.Label("Layer Options", EditorStyles.boldLabel);
                    EditorGUILayout.HelpBox(
                        "SemanticController is attached but has no active layers. " +
                        "Click Reinitialize or toggle the Renderer to trigger initialization.",
                        MessageType.Warning);
                    
                    using (new EditorGUI.DisabledScope(true))
                    {
                        foreach (var kv in gs.asset.layerInfo)
                            EditorGUILayout.LabelField($"Layer {kv.Key}", $"{kv.Value:N0} splats (loaded)");
                    }
                    
                    if (GUILayout.Button("Reinitialize SemanticController"))
                    {
                        semantic.InitializeFromAsset(gs.asset);
                        EditorUtility.SetDirty(semantic);
                    }
                }
                else
                {
                    // No SemanticController attached
                    GUILayout.Label("Layer Options", EditorStyles.boldLabel);
                    EditorGUILayout.HelpBox(
                        "Add a SemanticController component to this GameObject for semantic layer visibility and tint control.",
                        MessageType.Info);
                    ...
                    if (GUILayout.Button("Add SemanticController"))
                    {
                        Undo.AddComponent<SemanticController>(gs.gameObject);
                        EditorUtility.SetDirty(gs);
                    }
                }
            }
