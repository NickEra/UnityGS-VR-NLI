...
namespace GaussianSplatting.Editor
{
    ...
    public class GaussianSplatAssetCreator : EditorWindow
    {
        ...
        unsafe NativeArray<InputSplatData> LoadPLYSplatFile(string plyPath)
        {
            ...
            try
            {
                PLYFileReader.ReadFile(plyPath, out splatCount, out vertexStride, out attrNames, out attrSizes, out verticesRawData);
            }
            catch (Exception ex)
            {
            ...
            {
                // Perfect match with layered struct (standard PLY with int layer)
            ...
            {
                // Match with unlayered struct (standard PLY without layer)
            ...
            // ── Fallback: attribute-name-based parsing ──────────────────────
            // Handles non-standard PLY layouts, e.g. semantic PLY where:
            //   - field order differs from struct order (normals after opacity)
            //   - semantic_id is stored as short (2 bytes) instead of int (4 bytes)
            UnityEngine.Debug.Log($"PLY stride {vertexStride} does not match InputSplatData ({UnsafeUtility.SizeOf<InputSplatData>()}) " +
                $"or InputSplatDataUnlayered ({UnsafeUtility.SizeOf<InputSplatDataUnlayered>()}). " +
                $"Using attribute-name-based parsing.");

            // Build byte-offset map for each attribute
            var attrOffsets = new Dictionary<string, int>();
            {
                int offset = 0;
                for (int i = 0; i < attrNames.Count; i++)
                {
                    attrOffsets[attrNames[i]] = offset;
                    offset += attrSizes[i];
                }
            }

            // Verify we have the required attributes
            if (!attrOffsets.ContainsKey("x") || !attrOffsets.ContainsKey("f_dc_0") || !attrOffsets.ContainsKey("opacity") ||
                !attrOffsets.ContainsKey("scale_0") || !attrOffsets.ContainsKey("rot_0"))
            {
                m_ErrorMessage = $"PLY missing required attributes. Found: {string.Join(", ", attrNames)}";
                verticesRawData.Dispose();
                return data;
            }

            // Detect semantic_id attribute and its size
            bool hasSemanticId = attrOffsets.ContainsKey("semantic_id");
            int semanticIdOffset = hasSemanticId ? attrOffsets["semantic_id"] : -1;
            int semanticIdSize = 0;
            if (hasSemanticId)
            {
                int idx = attrNames.IndexOf("semantic_id");
                semanticIdSize = attrSizes[idx];
            }

            // Parse each vertex from raw bytes
            var result = new NativeArray<InputSplatData>(splatCount, Allocator.Persistent);
            byte* rawPtr = (byte*)verticesRawData.GetUnsafeReadOnlyPtr();

            // Pre-compute offsets for all needed fields
            int offX = attrOffsets["x"];
            int offDc0 = attrOffsets["f_dc_0"];
            int offOpacity = attrOffsets["opacity"];
            int offScale0 = attrOffsets["scale_0"];
            int offRot0 = attrOffsets["rot_0"];

            // Normals may or may not exist
            bool hasNormals = attrOffsets.ContainsKey("normal_0") || attrOffsets.ContainsKey("nx");
            int offNor0 = hasNormals
                ? (attrOffsets.ContainsKey("normal_0") ? attrOffsets["normal_0"] : attrOffsets["nx"])
                : -1;

            // SH rest coefficients
            bool hasSHRest = attrOffsets.ContainsKey("f_rest_0");
            int offSHRest0 = hasSHRest ? attrOffsets["f_rest_0"] : -1;

            for (int i = 0; i < splatCount; i++)
            {
                byte* vertPtr = rawPtr + (long)i * vertexStride;
                InputSplatData splat = default;

                // Position (always float3)
                splat.pos = *(Vector3*)(vertPtr + offX);

                // DC0 color (float3)
                splat.dc0 = *(Vector3*)(vertPtr + offDc0);

                // Normals (float3) – may not exist or may be at different position
                if (hasNormals && offNor0 >= 0)
                    splat.nor = *(Vector3*)(vertPtr + offNor0);
                else
                    splat.nor = Vector3.zero;

                // SH rest coefficients: PLY stores as [R0..R14, G0..G14, B0..B14]
                // Struct expects interleaved: sh1(R0,G0,B0), sh2(R1,G1,B1), ...
                if (hasSHRest)
                {
                    float* shSrc = (float*)(vertPtr + offSHRest0);
                    const int shCount = 15;
                    // Interleave from planar (RRR...GGG...BBB...) to (RGB, RGB, RGB, ...)
                    Vector3* shDst = &splat.sh1;
                    for (int j = 0; j < shCount; j++)
                    {
                        shDst[j] = new Vector3(
                            shSrc[j],                  // R channel
                            shSrc[j + shCount],        // G channel
                            shSrc[j + shCount * 2]     // B channel
                        );
                    }
                }

                // Opacity (float)
                splat.opacity = *(float*)(vertPtr + offOpacity);

                // Scale (float3)
                splat.scale = *(Vector3*)(vertPtr + offScale0);

                // Rotation (quaternion = float4)
                splat.rot = *(Quaternion*)(vertPtr + offRot0);

                // Layer / semantic_id
                if (hasSemanticId && semanticIdOffset >= 0)
                {
                    if (semanticIdSize == 2) // short
                        splat.layer = (int)(*(short*)(vertPtr + semanticIdOffset));
                    else if (semanticIdSize == 4) // int
                        splat.layer = *(int*)(vertPtr + semanticIdOffset);
                    else // fallback
                        splat.layer = 0;
                }
                else
            ...
            UnityEngine.Debug.Log($"Parsed {splatCount} splats via attribute-name-based path. " +
                $"hasNormals={hasNormals}, hasSHRest={hasSHRest}, hasSemanticId={hasSemanticId}");
