...
namespace GaussianSplatting.Editor.Utils
{
    ...
    {
        public static void ReadFileHeader(string filePath, out int vertexCount, out int vertexStride, out List<string> attrNames, out List<int> attrSizes)
        {
            vertexCount = 0;
            vertexStride = 0;
            attrNames = new List<string>();
            attrSizes = new List<int>();
            if (!File.Exists(filePath))
                return;
            using var fs = new FileStream(filePath, FileMode.Open, FileAccess.Read);
            ReadHeaderImpl(filePath, out vertexCount, out vertexStride, out attrNames, out attrSizes, fs);
        }

        // Overload without attrSizes for backward compatibility
        public static void ReadFileHeader(string filePath, out int vertexCount, out int vertexStride, out List<string> attrNames)
        {
            ReadFileHeader(filePath, out vertexCount, out vertexStride, out attrNames, out _);
        }

        static void ReadHeaderImpl(string filePath, out int vertexCount, out int vertexStride, out List<string> attrNames, out List<int> attrSizes, FileStream fs)
        {
        ...
        public static void ReadFile(string filePath, out int vertexCount, out int vertexStride, out List<string> attrNames, out List<int> attrSizes, out NativeArray<byte> vertices)
        {
            using var fs = new FileStream(filePath, FileMode.Open, FileAccess.Read);
            ReadHeaderImpl(filePath, out vertexCount, out vertexStride, out attrNames, out attrSizes, fs);
        ...
        {
            ReadFile(filePath, out vertexCount, out vertexStride, out attrNames, out _, out vertices);
        }
